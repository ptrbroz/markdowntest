...
At this moment, the communication is one-way. Each command sent via UART starts with a 3 byte opencode, followed by a number of data bytes and ends with a 3 byte closecode.
...
Each unit (or degree) corresponds to 1/360 of the signal period. Therefore, signal with duty set to 180 will have a 50% duty cycle. A channel with phase set to 90 will lag behind those with duty set to 0 by 1/4 of the signal period.
...
Therefore, we wish that the shift register contains following 9-bit integers in this order:
